...
Проект дополнен возможностями для анализа операций по описанию с помощью **регулярных выражений**:

### 🔍 Поиск по описанию
Пользователь может указать ключевое слово, и система отфильтрует операции, в чьём описании оно встречается (регистр не имеет значения).

Пример:
Введите слово: перевод
Найдено операций: 2

### 📊 Подсчёт по категориям
Можно указать список категорий (слов), и программа выведет статистику — сколько операций содержат каждую из этих категорий в описании.

Пример:
```python
Категории: ['перевод', 'оплата', 'открытие']
Результат: {'перевод': 3, 'оплата': 2, 'открытие': 1}
